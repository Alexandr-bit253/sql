...
int main() {
  ...
  try {
    ...
    std::string connStr = "host=" + std::string(host ? host : "localhost") +
                          " port=" + std::string(port ? port : "5435") +
    ...
    std::cout << "Choose search publisher method:\n"
              << "  1) by name\n"
              << "  2) by ID\n"
              << "Enter 1 or 2: ";
    std::string choice;
    ...
    if (choice == "1") {
      ...
      publisher =
          session.find<Publisher>().where("name = ?").bind(name).resultValue();
    } else if (choice == "2") {
      std::cout << "Enter ID: ";
      int id;
      std::cin >> id;
      publisher =
          session.find<Publisher>().where("id = ?").bind(id).resultValue();
    }
    ...
    for (const auto &book : publisher->books) {
      for (const auto &stock : book->stocks) {
        if (stock->shop && !stock->shop->name.empty()) {
          shops.insert(stock->shop->name);
        }
      }
    }

    std::cout << "\nShops with \"" << publisher->name << "\" books:\n";
